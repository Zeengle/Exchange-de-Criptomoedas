...
void menuinicial(pessoa pessoas[], float cotacaoBTC, float cotacaoETH,
                 float cotacaoXRP, int usuariologado) {
  limpaterminal();
  char opc;
  while (1) {
    printf(" |---------------[Menu Inicial]---------------| \n");
...
void menu(pessoa pessoas[], float cotacaoBTC, float cotacaoETH,
          float cotacaoXRP, int usuariologado) {
  limpaterminal();
  char opc;
  while (1) {
...
void cadastrar(pessoa pessoas[], float cotacaoBTC, float cotacaoETH,
               float cotacaoXRP, int usuariologado) {
  limpaterminal();
  char cpfcadastro[12];
...
void login(pessoa pessoas[], float cotacaoBTC, float cotacaoETH,
           float cotacaoXRP, int usuariologado) {
  limpaterminal();
  char cpflogin[12];
  char senhalogin[7];
...
void depositar(pessoa pessoas[], float cotacaoBTC, float cotacaoETH,
               float cotacaoXRP, int usuariologado) {
  limpaterminal(); // funciona (implementar no manip dps) e ver o tempo sertin
                   // de colocar
  float depositado = 0;
  printf("|------------------------------------------------[Depositar]---------"
         "---------------------------------------| \n");
  ...
  if (depositado < 0) { // talvez eu tenha levado a sério demais
    printf(
        "Vai toma no cu seu filho da puta para de ser burro aq é depósito seu "
        "animal bota um valor de gnt arrombado do krl toma nesse seu cu");
  }
...
void sacar(pessoa pessoas[], float cotacaoBTC, float cotacaoETH,
           float cotacaoXRP, int usuariologado) {
  limpaterminal();
  float sacado = 0;
  printf("|------------------------------------------------[Sacar]-------------"
...
void vender(pessoa pessoas[], float cotacaoBTC, float cotacaoETH,
            float cotacaoXRP, int usuariologado) {
  limpaterminal();
  char escolha;
...
// Função que faz a atualização das cotações de cada moeda variarem até +5% ou
// -5%
void atualizar(pessoa pessoas[], float cotacaoBTC, float cotacaoETH,
               float cotacaoXRP, int usuariologado) {
  limpaterminal();
  srand(time(NULL));
...
// Função que faz a verificação da senha do usuário logado para permitir a
// execução de certas ações
